...
<script>
import { helpList, helpAdd, helpUpdate, helpDelete, getHelp } from '@/api/help.js'
...
export default {
  ...
  methods: {
    getList() {
      ...
      helpList(this.listQuery).then(res => {
        if (res.code == 20000) {
    ...
    handleEdit() {
      ...
      helpUpdate(this.updateData).then(res => {
        if (res.code == 20000) {
          this.$message({
            type: 'success',
            message: res.msg
