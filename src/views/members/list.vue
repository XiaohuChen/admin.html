<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="listQuery.IsFreeze" placeholder="是否冻结" clearable style="width: 150px" class="filter-item">
        <el-option v-for="(item, index) in {0:'未冻结',1:'被冻结',100:'全部'}" :key="index" :label="item" :value="index" />
      </el-select>
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;">
      ...
      <el-table-column label="冻结状态" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.IsFreeze == 0" style="color: green;">正常</span>
      ...
      <el-table-column v-permission="['/members/membersStatus', '/members/subList', '/members/holdCoin']" label="操作"
        width="320" align="center">
        <template slot-scope="scope">
          <div>
            <span v-permission="['/members/membersStatus']">
              <el-button size="small" :class="[scope.row.Status == 1 ? 'el-button--danger' : 'el-button--primary']"
            ...
            <span v-permission="['/members/membersStatus']">
              <el-button size="small" :class="[scope.row.Status == 1 ? 'el-button--danger' : 'el-button--primary']"
                @click="getSetStatus(scope.row,1)">
                <span v-if="scope.row.IsFreeze == 0">冻结</span>
                <span v-else>解封</span>
              </el-button>
            </span>
    ...
    <el-dialog title="我的下级会员" :visible.sync="subListVisible">
      ...
      <pagination v-show="total > 0" :total="subtotal" :page.sync="sublistQuery.page" :limit.sync="sublistQuery.limit"
    ...
    <el-dialog title="会员持币信息" :visible.sync="coinVisible">
      ...
      <el-table :key="tableKey" v-loading="listLoading" :data="holdCoinlist" border fit highlight-current-row style="width: 100%;">
        ...
        <el-table-column v-permission="['/members/memberCoinUpdate', '/members/memberCoinLockMoney']" label="修改操作"
          ...
          <template slot-scope="scope">
            <div>
              ...
              <span v-permission="['/members/memberCoinLockMoney']" v-if="scope.row.CoinName !== 'IA'">
